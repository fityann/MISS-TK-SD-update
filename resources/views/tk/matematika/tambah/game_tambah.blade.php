...
<head>
    ...
    <title>Kuis Penjumlahan TK</title>
...
<body class="bg-game min-h-screen flex flex-col font-sans relative overflow-x-hidden">
    ...
    <div class="flex-grow flex items-center justify-center px-4 pt-20 pb-10">
        ...
        <div class="relative w-full max-w-lg bg-[#FFF9E3] rounded-[40px] border-[10px] border-[#D28E43] shadow-2xl p-6 text-center">
            ...
            <div class="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-red-600 text-white px-8 py-2 rounded-xl shadow-lg font-bold text-xl uppercase tracking-wider z-20">
                Penjumlahan
            </div>
    ...
    <script>
        ...
        function loadQuestion() {
            ...
            Array.from(choices).sort(() => Math.random() - 0.5).forEach((val, i) => {
                ...
                btn.textContent = val;
                btn.dataset.value = val; // Simpan nilai di dataset untuk dicari nanti
        ...
        function check(val, btn) {
            ...
            if (val === answer) {
                ...
                // EFEK SALAH: Tombol yang diklik jadi merah
                btn.classList.replace(btn.classList[0], 'bg-red-600');
                btn.classList.add('opacity-50', 'grayscale');
                document.getElementById('sWrong').play();

                // VALIDASI TAMBAHAN: Cari tombol yang benar, lalu buat bergoyang
                btns.forEach(b => {
            ...
            setTimeout(() => { currentQ++; loadQuestion(); }, 1500); // Jeda sedikit lebih lama biar terlihat
        }
        ...
        function showFinish() {
            ...
            fetch('/leaderboard/save', {
                ...
                body: JSON.stringify({ quiz_type: "tk_penjumlahan", score: score })
